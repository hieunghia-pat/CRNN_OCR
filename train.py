...
from model.crnn import make_model
import os
from loss_utils.CTCLoss import CTCLoss, SimpleLossCompute, NoamOpt
...
def train():
    ...
    model = make_model(vocab_size=len(vocab.stoi), image_height=config.image_size[-1], d_model=config.d_model, dropout=config.dropout)

    model.cuda()
    criterion = CTCLoss(padding_idx=vocab.padding_idx)
    criterion.cuda()
    model_opt = NoamOpt(config.d_model, 1, 2000,
